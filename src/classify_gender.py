...
def main():
    ...
    print("score before")
    clf_before, score_test_before, score_train_before = train_clf(x_train, y_train_gender, x_dev, y_dev_gender)

    if not args.inlp_mat:
        mat_used = "nomat"
        pickle.dump(clf_before, open("../data/classifiers/clf_gender_{}_before_{}".format(args.lang, mat_used), "wb"))
        with open("../data/classifiers/results.txt", "a") as f:
            f.write("classify gender, params:" + str(args) + "\n")
    ...
    x_dev_inlp = x_dev.dot(P)

    print("after")
    clf_after, score_test_after, score_train_after = train_clf(x_train_inlp, y_train_gender, x_dev_inlp, y_dev_gender)

    mat_used = args.inlp_mat.rsplit("/",1)[1].split(".")[0]

    pickle.dump(clf_before, open("../data/classifiers/clf_gender_{}_before_{}".format(args.lang, mat_used), "wb"))
    pickle.dump(clf_after, open("../data/classifiers/clf_gender_{}_after_{}".format(args.lang, mat_used), "wb"))
    ...
    with open("../data/classifiers/results_gender.txt", "a") as f:
