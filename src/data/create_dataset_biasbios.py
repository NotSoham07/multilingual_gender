...
def preprocess(data: List[dict], lang):
...
def split_train_dev_test(data: List[dict], output_dir: str, vocab_size: int, lang: str):
        """
        writes the dataset to files
        """
        g2i, i2g = {"m": 0, "f": 1}, {1: "f", 0: "m"}
        all_profs = list(sorted(set([d["raw_title"] for d in data])))
        if lang == "es" or lang == "fr" or lang == "de":
                all_profs = list(sorted(set([d["title"] for d in data])))
        all_words = []
        ...
        for entry in tqdm.tqdm(data, total=len(data)):
                ...
                if lang == "es" or lang == "fr" or lang == "de":
                        gender, prof = entry["gender"].lower(), entry["title"].lower()
        ...
        for prof in all_profs:
                ...
                print(len(relevant_prof))

                if (lang == "es" or lang == "fr" or lang == "de") and len(relevant_prof) < 500:
                       continue
...
def main():
        ...
        preprocess(data, args.lang)

        split_train_dev_test(data = data, output_dir = args.output_dir, vocab_size = args.vocab_size, lang = args.lang)
